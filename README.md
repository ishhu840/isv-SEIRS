...
A single release of **25,000 CFAV-positive male *Aedes aegypti* in early March** is projected to reduce annual dengue burden in Rawalpindi by a **median 91.4 %** (95 % CI 54.6–96.7 %). The result is driven by well-measured CFAV biology rather than by uncertain ecological assumptions.
...
8. [Step 4 — Adding CFAV vertical transmission](#8-step-4--adding-cfav-vertical-transmission)
9. [Step 5 — Closing the loop (coupled human–vector engine)](#9-step-5--closing-the-loop)
10. [Step 6 — Can CFAV establish? (R₀_ISV via NGM)](#10-step-6--can-cfav-establish)
...
**Insect-Specific Viruses (ISVs)** are viruses that infect mosquitoes but cannot replicate in humans or vertebrates. One ISV in particular — **CFAV (Cell-Fusing Agent Virus)** — has two properties that make it interesting for dengue control:

1. **CFAV blocks dengue replication inside the mosquito gut.** A mosquito that already carries CFAV is much less able to transmit dengue when it later bites an infected person. This is "super-infection exclusion".
2. **CFAV spreads naturally between mosquitoes.** Infected females pass it to their eggs (maternal route). Infected males pass it through mating to either their offspring (paternal route) or the female herself (venereal route). So once CFAV is introduced into a population, it can spread on its own without ongoing intervention.

→ The intervention idea is: release CFAV-carrying *Aedes aegypti* males before dengue season. They mate with wild females, CFAV spreads through the wild population by natural mating, and the mosquito population becomes much less effective at transmitting dengue by the time the outbreak season arrives.
...
- **Q1 — Can CFAV establish in Rawalpindi's climate?** That is, will the virus self-sustain at local temperatures (R₀_ISV > 1) or die out?
...
→ To answer these we need a model that combines climate, mosquito biology, dengue dynamics, CFAV biology, and an intervention layer. The next sections build this model **step by step**.
...
→ Now we have a working wild-mosquito model. Next we add the CFAV biology layer. That is Step 4.

---

## 8. Step 4 — Adding CFAV vertical transmission
...
| Paternal | ν_P | **0.76** | An infected male mates with a wild female; 76 % of her offspring inherit CFAV through his sperm. |
| Venereal | ν_V | **0.31** | An infected male mates with a wild female; 31 % of those females themselves become CFAV-positive in their body. |

These three routes are combined into one **composite per-offspring infection probability** `ν_eff` weighted by how many males vs females are currently CFAV-positive:
...
**ν_eff** is then applied at adult emergence: a fraction `ν_eff` of new recruits joins the CFAV-infected female pool N_I, and `(1 − ν_eff)` joins the wild susceptible pool S_W. The male side splits the same way.

Implementation: [`01_Scripts/04_ISV_Mosquito_Dynamics.R`](./01_Scripts/04_ISV_Mosquito_Dynamics.R)

→ Now we have humans and mosquitoes both modelled, and CFAV biology layered in. The final modelling step is to **couple them**. That is Step 5.
...
This closes the classic **Ross-Macdonald vector-host loop**. The CFAV intervention enters as a **blocking shield** on the human side:

$$E_{new}(t) = \beta_t \cdot \frac{S_H \cdot I_H}{N_H} \cdot (1 - \varepsilon \cdot p_{ISV}(t)) + \lambda$$

where `ε` is the per-mosquito blocking efficacy and `p_ISV(t)` is the fraction of wild adult females that are CFAV-positive at time t. As CFAV spreads through the wild mosquito population, `p_ISV` rises, the shield `(1 − ε·p_ISV)` shrinks the force of infection, and human cases fall.
...
*Figure 1. Integrated SEIRS-ISV compartment diagram. Blue: human SEIRS. Orange: adult female mosquitoes (wild S_W → E_W → I_W and CFAV-positive N_I). Green: aquatic stage and males. Solid arrows: dengue and life-cycle flows. Dashed arrows: CFAV transmission. The `BLOCKS (ε)` arrow from N_I shows the dengue super-infection-exclusion shield.*

→ With the full coupled model in place, we can now ask the establishment question: will CFAV self-sustain? That is Step 6.

---

## 10. Step 6 — Can CFAV establish?

This is the critical biological question. If R₀_ISV < 1 at local temperatures, CFAV dies out after the release and we waste the entire effort. If R₀_ISV > 1, CFAV spreads on its own and the intervention can succeed.
...
*Figure 2. Per-generation R₀_ISV as a function of temperature. The horizontal line at R₀_ISV = 1 marks the establishment threshold. CFAV self-establishes for T > T_c = 20.3 °C. At Rawalpindi's mean of 21.9 °C, R₀_ISV = 1.21; maximum 1.42 at the thermal optimum.*

**The answer to Q1 is yes:** CFAV can self-establish in Rawalpindi's climate throughout the warm season.
...
If the projected case reduction comes mostly from uncertain ecological assumptions (carrying capacity, initial population sizes), the result is fragile. If it comes mostly from **well-measured biological parameters** (CFAV blocking efficacy, vertical transmission rates), the result is robust.
...
|---|---|---|
| ε (blocking efficacy) | **0.98** | Strongest driver. Higher CFAV blocking → bigger case reduction. |
| ν_M (maternal transmission) | **0.96** | Second-strongest. Faster CFAV spread through eggs → bigger reduction. |
...
### 13.2 CFAV establishes above 20.3 °C (Q1 answered)
...
*Figure 6. Weekly temperature (orange, left axis) and per-generation R₀_ISV (blue, right axis), with release weeks coloured by Monte Carlo efficacy. The optimal release window sits in the pre-monsoon shoulder — temperature is just above the CFAV establishment threshold, and CFAV has months to spread through the wild population before the outbreak season starts.*

**Why pre-monsoon wins** — and this is the key insight — is **not** because R₀_ISV is highest in March. It is because CFAV needs **time to spread**. A March release gives CFAV ~5–6 generations of mating-driven spread before the dengue outbreak begins. A July release gives it almost none.
...
> A single, well-timed **pre-monsoon release of 25,000 CFAV-positive male *Ae. aegypti*** in early March is projected to reduce annual dengue burden in Rawalpindi by a **median 91.4 %**, driven by well-characterised CFAV biology.
...
### 15.4 Adult mosquito dynamics — wild vs CFAV-infected females
...
| ε | Beta(2,2) on [0.05, 0.95] | — | CFAV dengue-blocking efficacy | Baidaliuk 2019 [4] |
...
### 16.5 CFAV transmission (Logan 2022 [7])
...
| Symbol | Value | Meaning |
|---|---|---|
| N_release | **25,000** | CFAV-positive males released in a single pulse |
...
│   ├── 04_ISV_Mosquito_Dynamics.R       CFAV transmission kernel (Step 4)
