...
df_all$ISV_Cases_Aug <- 0
YEARS <- 2013:2024
K0 <- 1e6; k_R_mosq <- 0.02; M0_F <- 20000; I0W_f <- 0.001
N_rel_M <- 25000L
rw <- 31L # August Release (Week 31)
eps_med <- 0.80
...
for (yr in YEARS) {
  ...
  df_all$ISV_Cases_Aug[mask_yr] <- pp[mask_yr]
}
yr_sums <- aggregate(cbind(Baseline, ISV_Cases_Aug) ~ Year, data=df_all, sum)
yr_sums$Reduction_Aug <- 100 * (yr_sums$Baseline - yr_sums$ISV_Cases_Aug) / yr_sums$Baseline
print(yr_sums)
